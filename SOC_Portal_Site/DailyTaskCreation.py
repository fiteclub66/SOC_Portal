...
def dailyUpdate():
    ...
    query = ("SELECT * FROM (SELECT DISTINCT taskName, min(counter) as mincounter FROM UpcomingTasks WHERE freqType = ('Daily') GROUP BY taskName) AS x INNER JOIN UpcomingTasks as f on f.taskName = x.taskName and f.counter = x.mincounter")
    cur.execute(query)
    ...
    cur.close()  
    db.close()
    return (("Daily done at "+str(datetime.datetime.now())))
